				...
				if(level[row][column] != null) {
					if(level[row][column].toCharArray()[0] == 'p') {
						...
						/**myEntities.add(new Entity(
								...
								column * 32, row * 32));*/
						String enemy = enemies[Integer.valueOf(""+level[row][column].toCharArray()[1])];
						if(enemy.equals(enemies[0])) {
							myEntities.add(new Entity(enemy, column * 32, (row * 32) - 10));
						} else {
							myEntities.add(new Entity(enemy, column * 32, (row * 32)));
						}
						level[row][column] = null;
	...
	private static void parsePlatforms(String[][] level) {
		while(!checkArrayEmpty(level)) {
			...
			while(platform == null && column < level[0].length && row < level.length) {
				...
				//System.out.println("Checking value:" + level[row][column]);
